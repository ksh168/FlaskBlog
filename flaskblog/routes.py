...
def register():
	...
	if form.validate_on_submit():
		...
		#creating user so as to add to db
...
def post(post_id):
	post = Post.query.get_or_404(post_id)	# either return the post or 404
...
def update_post(post_id):
	post = Post.query.get_or_404(post_id)	# either return the post or 404
	#only user who created this post should be able to update it
	...
	if form.validate_on_submit():
		...
		#no need to add now as they already in the db
		db.session.commit()
...
def user_posts(username):
	...
	user = User.query.filter_by(username=username).first_or_404()	#get first user with this username or return 404
	
	posts_by_username = Post.query.filter_by(author=user)
	posts_ordered_by_latest_first = posts_by_username.order_by(Post.date_posted.desc())
	posts = posts_ordered_by_latest_first.paginate(page=page, per_page=5)
...
def reset_request():
	#to make sure user is logged out before trying to reset password
	if current_user.is_authenticated:
		return redirect(url_for('home'))
